...
for line in postags:
    ...
    if ('A' in postagsent) and idiomset[1] == '1':
        print parsedSociallists[i]
        stags+=1
    elif ('A' in postagsent) and idiomset[1] == '0':
        itags+=1
    i+=1

print "Probablity of Social Lists containing Adjectives is ",str(stags)+'/'+str(scount),str(float(stags)/scount)
print "Probablity of Non-Social Lists containing Adjectives is ",str(itags)+'/'+str(i-scount),str(float(itags)/(i-scount))